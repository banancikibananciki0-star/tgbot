...
def require_auth(func):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not context.args or not check_password(context.args[0]):
            await update.message.reply_text("❌ Неверный пароль!")
            return
        # Убираем пароль из аргументов для основной функции
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🤖 Бот для управления Xray запущен!\n\n"
        "Доступные команды:\n"
        "/add_vmess пароль email [порт] - Добавить VMESS пользователя\n"
        "/list_users - Список пользователей\n"
        "/change_password старый_пароль новый_пароль - Сменить пароль\n"
        "/restart_xray пароль - Перезапустить Xray\n\n"
        f"💡 Дефолтный пароль: {config.ADMIN_PASSWORD}"
    )

@require_auth
async def add_vmess(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Добавление VMESS пользователя"""
    if len(context.args) < 1:
        await update.message.reply_text("❌ Использование: /add_vmess пароль <email> [порт]")
        return
...
async def change_password_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 2:
        await update.message.reply_text("❌ Использование: /change_password <старый_пароль> <новый_пароль>")
        return
    
    old_password, new_password = context.args
    
    if not check_password(old_password):
        await update.message.reply_text("❌ Неверный старый пароль!")
        return
